...
class TestWydarzenie(unittest.TestCase):
    ...
    def testCorrectAddSearchDelete(self):
        ...
        wyd1["id"] = dbops.dodaj_wydarzenie(eng, wyd1["nazwa"], wyd1["data_rozp"], wyd1["godzina_rozp"], wyd1["data_zak"],wyd1["godzina_zak"], wyd1["opis"])
        wyd2["id"] = dbops.dodaj_wydarzenie(eng, wyd2["nazwa"], wyd2["data_rozp"], wyd2["godzina_rozp"], wyd2["data_zak"],wyd2["godzina_zak"], wyd2["opis"])
    ...
    def testIncorrectAdd(self):
        eng = create_engine(dbops.dbpath, echo=False)
        self.assertRaises(ValueError, dbops.dodaj_wydarzenie, eng, "niemożliwe", "2024-01-01", "00:00", "2023-12-31", "23:59", "podróż w czasie")
        eng.dispose()
    
    def testMod(self):
        eng = create_engine(dbops.dbpath, echo=False)
        id = dbops.dodaj_wydarzenie(eng, "wykład", "2024-01-14", "14:15", "2024-01-14", "16:00", "systemy typów")
        dbops.mod_wydarzenie(eng, id, "wykład z systemów typów", None, None, None, None, "wykładowca Piotr Polesiuk")
...
class TestOsoba(unittest.TestCase):
    ...
    def testCorrectAddSearch(self):
        eng = create_engine(dbops.dbpath, echo=False)
        id = dbops.dodaj_osoba(eng, "Ferdynand Kiepski", "ferdek@kiepski.pl")
    ...
    def testIncorrectAdd(self):
        eng = create_engine(dbops.dbpath, echo=False)
        self.assertRaises(ValueError, dbops.dodaj_osoba, eng, "Ferdynand", "ferdek")
        eng.dispose()
    
    def testSignupQuit(self):
        eng = create_engine(dbops.dbpath, echo=False)
        wyd_id = dbops.dodaj_wydarzenie(eng, "libacja", "2024-01-13", "20:00", "2024-01-14", "05:00", "UWAGA. godzina zakończenia jest umowna")
